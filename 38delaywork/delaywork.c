...
irqreturn_t handler(int irq, void *args)
{
    ...
    queue_delayed_work(selfworkqueue, &selfwork, 3 * HZ);
    return IRQ_HANDLED;
}

// 入口函数
static int delaywork_init(void)
{
    ...
    *(gpio_dir) = input;
    ...
    // 初始化工作队列
    INIT_DELAYED_WORK(&selfwork, workfunction);

    return 0;
}

static void delaywork_exit(void)
{
    cancel_delayed_work_sync(&selfwork);      // 取消工作
