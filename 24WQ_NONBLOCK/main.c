...
ssize_t MyRead(struct file *file, char __user *buf, size_t size, loff_t *offset)
{
    ...
    if(file->f_flags & O_NONBLOCK){   // 判断是否有文件数据
...
static int test_init(void)
{
    ...
    if (ret < 0)
        printk("WQ_NONBLOCK add is faild...\n");
    printk("WQ_NONBLOCK add is success...\n");

    // 创建设备节点 （作为桥梁连接上层和底层之间交互）
    MyDevices1.myclass = class_create(THIS_MODULE, "WQ_NONBLOCK");
    device_create(MyDevices1.myclass, NULL, MyDevices1.dev, NULL, "WQ_NONBLOCK");
