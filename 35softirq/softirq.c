...
void myaction(struct softirq_action *softirq_action)
{
    printk("softirq action\n");
}
...
// 入口函数
static int mysoftirq_init(void)
{
    ...
    *(gpio_dir) = input;
    ...
    open_softirq(MY_SOFTIRQ, myaction);

    return 0;
}

static void mysoftirq_exit(void)
{
    free_irq(irq, NULL);
    iounmap(gpio_dr);
    iounmap(gpio_dir);
