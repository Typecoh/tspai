...
static int test_init(void)
{
    ...
    if (ret < 0)
        printk("MUTEX add is faild...\n");
    printk("MUTEX add is success...\n");

    // 创建设备节点 （作为桥梁连接上层和底层之间交互）
    MyDevices1.myclass = class_create(THIS_MODULE, "MUTEX");
    device_create(MyDevices1.myclass, NULL, MyDevices1.dev, NULL, "MUTEX");
