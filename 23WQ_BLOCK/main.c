...
ssize_t MyRead(struct file *file, char __user *buf, size_t size, loff_t *offset)
{
    ...
    wait_event_interruptible(read_wq, readDevices->condition);
...
static int test_init(void)
{
    ...
    if (ret < 0)
        printk("WQ_BLOCK add is faild...\n");
    printk("WQ_BLOCK add is success...\n");

    // 创建设备节点 （作为桥梁连接上层和底层之间交互）
    MyDevices1.myclass = class_create(THIS_MODULE, "WQ_BLOCK");
    device_create(MyDevices1.myclass, NULL, MyDevices1.dev, NULL, "WQ_BLOCK");

    MyDevices1.condition = 0;
    return 0;
}
