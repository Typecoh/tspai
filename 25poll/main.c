...
ssize_t MyRead(struct file *file, char __user *buf, size_t size, loff_t *offset)
{
    ...
    // {                               // 判断是否有文件数据
    //     if (readDevices->flag != 1) // 如果没有文件数据
    //         return -EAGAIN;         // 再次查询
    // }

    // wait_event_interruptible(read_wq, readDevices->flag);
...
static __poll_t Mypoll(struct file *file, struct poll_table_struct *p)
{
    struct devices *test = (struct devices *)file->private_data;

    __poll_t mask = 0;
    poll_wait(file, &read_wq, p);   // 在这个地方进行等待释放了 cpu资源
    if (test->flag == 1)
        mask |= POLLIN;
    return mask;
}
...
static int test_init(void)
{
    ...
    if (ret < 0)
        printk("POLL add is faild...\n");
    printk("POLL add is success...\n");

    // 创建设备节点 （作为桥梁连接上层和底层之间交互）
    MyDevices1.myclass = class_create(THIS_MODULE, "POLL");
    device_create(MyDevices1.myclass, NULL, MyDevices1.dev, NULL, "POLL");
